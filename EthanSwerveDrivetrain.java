 ...
 */
public class EthanSwerveDrivetrain {
    ...
    /* TODO: EDIT FROM ORIGINAL
     * Addition of replacement fields
     */
    protected final Supplier<Double> better_yawGetter;
    protected final Supplier<Double> better_angularVelocity;
    protected final Supplier<Double> timeSinceLastGyroUpdate;
    ...
    /* TODO EDIT FROM ORIGINAL
     * Removal because it's relient on pigeon2
     */
     ...
     */
    public static class EthanSwerveDrivetrain {
    ...
    public class OdometryThread {
         ...
         * <b>EDIT NOTE</b>
         * <br></br>
         * getSignals was not visable, so replaced with functionaly
         * same code from constructor of swerveModule class
         * {@link SwerveModule#SwerveModule()}
         * <br></br>
         * <b>EDIT NOTE</b>
         * <br></br>
         * {@link MySwerveDrivetrain#m_yawGetter}
         * and 
         * {@link MySwerveDrivetrain#m_angularVelocity}
         * removed to avoid errors from lack of pigeon2
         */
        public OdometryThread() {
            ...
            m_allSignals = new BaseStatusSignal[(ModuleCount * 4)];
            for (int i = 0; i < ModuleCount; ++i) {
                /* TODO: EDIT FROM ORIGINAL
                 * getSignals was not visable, so replaced with functionaly same code
                 * from constructor of swerveModule class
                 * {@link SwerveModule#SwerveModule()}
                 */
                ...
                m_allSignals[(i * 4) + 0] = Modules[i].getDriveMotor().getPosition().clone();
                m_allSignals[(i * 4) + 1] = Modules[i].getDriveMotor().getVelocity().clone();
                m_allSignals[(i * 4) + 2] = Modules[i].getSteerMotor().getPosition().clone();
                m_allSignals[(i * 4) + 3] = Modules[i].getSteerMotor().getVelocity().clone();
            }
            /* TODO: EDIT FROM ORIGINAL
             * These signals from pigeon2 are removed
             */
         ...
         * <b>EDIT NOTE</b>
         * <br></br>
         * definition of local variable {@code yawDegrees}
         * redefined with {@link OdometryThread#compensateForLatency()}
         * to replace 
        ...
        public void run() {
            ...
            while (m_running) {
                ...
                try {
                    ...
                    /* TODO: EDIT FROM ORIGINAL
                     * Original method replaced so as to achive same affect
                     */
                    // double yawDegrees = BaseStatusSignal.getLatencyCompensatedValue(
                            // m_yawGetter, m_angularVelocity);
                    double yawDegrees = compensateForLatency(
                        better_yawGetter.get(), 
                        better_angularVelocity.get(),
                        timeSinceLastGyroUpdate.get()
                    );
         ...
         * Replacement for
         * {@link BaseStatusSignal#getLatencyCompensatedValue()}
         * @param currentVal the value at time of last update
         * @param rateOfChangeForVal the rate of change or slope at time of last update
         * @param timeSinceLastUpdate the amount of time passed since the value was aquired
         * @return the compensated value
          */
        private static double compensateForLatency(double currentVal, double rateOfChangeForVal, double timeSinceChangeInVal){
            ...
            if ( timeSinceChangeInVal > 0.300) {
                timeSinceChangeInVal = 0.300;
                //This part was in the original method usage, so I'm keeping it
            }
            return currentVal + (rateOfChangeForVal * timeSinceChangeInVal);
    ...
    public MySwerveDrivetrain(
         ...
         */
        better_yawGetter = null;
        better_angularVelocity = null;
        timeSinceLastGyroUpdate = null;

        /* TODO EDIT FROM ORIGINAL
         * Eradication of pigeon2 usage
         */
        ...
        //     if (!response.isOK()) {
        //         System.out.println(
        //                 "Pigeon2 ID " + m_pigeon2.getDeviceID() + " failed config with error: " + response.toString());
        //     }
        // }
    ...
    public void tareEverything() {
        try {
            ...
            /* TODO EDIT FROM ORIGNIAL
             * Replacement of yawGetter
             */
            // m_odometry.resetPosition(Rotation2d.fromDegrees(m_yawGetter.getValue()), m_modulePositions, new Pose2d());
            m_odometry.resetPosition(Rotation2d.fromDegrees(better_yawGetter.get()), m_modulePositions, new Pose2d());
        } finally {
            m_stateLock.writeLock().unlock();
        }
    }
    ...
    public void seedFieldRelative(Pose2d location) {
        try {
            ...
            m_odometry.resetPosition(Rotation2d.fromDegrees(better_yawGetter.get()), m_modulePositions, location);
     ...
     *
     * <br></br>
     * <b>EDIT NOTE</b> No longer in use
     * @deprecated
     * 
